...
;(function($) {
    H.lottery = {
        ...
        event: function() {
          ...
          $('body').delegate("footer div", "touchend", function(e){
            e.preventDefault();
            if($(this).attr("type") == "card"){
              toUrl('card.html');
              return;
            }
            toUrl('recommend.html?type='+ $(this).attr('type'));
          });
        },
        ...
        nowCountdown: function(prizeActList){
            ...
            beginTimeLong += me.dec;
            me.$lotteryCountdown.find('.detail-countdown').attr('etime',beginTimeLong).empty().removeClass("none");
            me.$lotteryCountdown.find(".countdown-tip").html("<img src='"+ End +"'/>");
        ...
        count_down: function() {
            ...
            me.$lotteryCountdown.find('.detail-countdown').each(function() {//摇奖页倒计时
                $(this).countDown({
                    etpl : '<label>%H%</label>' + '<span><img src="'+ Dot +'"></span>' + '<label>%M%</label>' + '<span><img src="'+ Dot +'"></span>' + '<label>%S%</label>', // 还有...结束
                    stpl : '<label>%H%</label>' + '<span><img src="'+ Dot +'"></span>' + '<label>%M%</label>' + '<span><img src="'+ Dot +'"></span>' + '<label>%S%</label>', // 还有...开始
                    sdtpl: '',
                    otpl: '',
                    otCallback: function() {
                        if(me.canJump){
        ...
        change: function() {
          ...
          me.$lotteryCountdown.addClass('none').find(".countdown-tip").html('摇奖已结束,下期再战！');
        ...
        if(data.result){
            ...
            if(list && list.length > 0){
                ...
                for(var i = 0 ; i < list.length; i++){
                    con += "<li>" + (list[i].ni || "匿名用户") + "   中了  <span>" + list[i].pn + "</span></li>";
                }
