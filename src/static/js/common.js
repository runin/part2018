...
var showTips = function(tips, timer) {
    ...
    if (showTipsFlag) {
        showTipsFlag = false;
        $('body').append('<div class="showTips" style="position:fixed;max-width:80%;z-index:999999999999;color:#000;padding:19px 38px;font-size:24px;border-radius:3px;background:rgba(255,255,255,.9);text-align:center;opacity:0">' + tips + '</div>');
    ...
    var runTime = function() {
        ...
        vthis.each(function() {
            ...
            var showTime = function(rT, showTpl) {
                ...
                var hString = h_.toString().split('');
                var mString = m_.toString().split('');
                var sString = s_.toString().split('');
                ...
                nthis.html(showTpl.replace(/%S%/, '<i>'+sString[0]+'</i><i>'+sString[1]+'</i>').replace(/%M%/,  '<i>'+mString[0]+'</i><i>'+mString[1]+'</i>').replace(/%H%/,  '<i>'+hString[0]+'</i><i>'+hString[1]+'</i>').replace(/%D%/, d_).replace(/%SS%/, ss_));
...
$(function() {
  ...
  window.bottomBar = function (hide) {
    var $body = $('body');
    if(hide) return;
    $('<footer class="horizontal" data-type=""><div class="jctj" type="jctj"><span></span></div><div class="jmd" type="jmd"><span></span></div><div class="card" type="card"><span></span></div></footer>').appendTo($body);
  };
  if(/lottery|recommend|card/.test(location.pathname)){
    W.bottomBar();
  }
